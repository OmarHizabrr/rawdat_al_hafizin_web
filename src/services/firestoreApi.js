...
class FirestoreApi {
  ...
  getConversationsInboxQuery(userId) {
    return query(this.getConversationsCollection(), where('participants', 'array-contains', userId))
  }
  ...
  getConversationMessagesQuery(conversationId) {
    return query(this.getConversationMessagesCollection(conversationId), orderBy('createdAt', 'asc'))
  }
