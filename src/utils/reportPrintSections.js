    ...
    if (reportData.halakaAttendance?.length) {
      sections.push({
        ...
        rows: reportData.halakaAttendance.map((r) => ({
          ...
          ...formatHalakaAttendanceDisplay(r),
  ...
  if (reportData.kind === 'halaka') {
    if (reportData.sessionStudentRows?.length) {
      sections.push({
        ...
        introMeta: buildHalakaReportHeaderItems(reportData),
        columns: HALAKA_SESSION_STUDENT_TABLE_COLUMNS,
        rows: formatHalakaSessionStudentRowsForDisplay(reportData.sessionStudentRows, {
          formatDateTime: fmt,
        }),
      })
    }
